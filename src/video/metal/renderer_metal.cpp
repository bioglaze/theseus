...
void UpdateUBO( const float localToClip[ 16 ], const float localToView[ 16 ], const float localToShadowClip[ 16 ],
                const ShaderParams& shaderParams, const Vec4& lightDir )
{
...
void BeginRendering( teTexture2D& color, teTexture2D& depth, teClearFlag clearFlag, const float* clearColor )
{
    if (color.index != -1 && depth.index != -1)
    {
        ...
        renderer.renderPassDescriptorFBO->depthAttachment()->setClearDepth( 0 );
...
{
    MTL::Texture* textures[] = { TextureGetMetalTexture( textureIndex ), TextureGetMetalTexture( textureIndex ), TextureGetMetalTexture( textureIndex ) };
...
{
    float m[ 16 ];
    UpdateUBO( m, m, m, shaderParams, Vec4( 0, 0, 0, 1 ) );
...
void teUIDrawCall( const teShader& shader, const teTexture2D& fontTex, int displaySizeX, int displaySizeY, int scissorX, int scissorY, unsigned scissorW, unsigned scissorH, unsigned elementCount, unsigned indexOffset, unsigned vertexOffset )
{
    ...
    UpdateUBO( localToClip.m, localToClip.m, localToClip.m, shaderParams, Vec4( 0, 0, 0, 1 ) );
